...
Do not blindly crank both hands to the right because a checklist told you to. Hit three balls after each small change and watch the **start direction**. A grip adjustment that turns a slice into a low pull-hook is not “progress” you should keep exaggerating; it means the face changed faster than the path.

## Match the Fix to the Ball Flight

| Shot shape for a right-handed golfer | Start with |
|---|---|
| Starts left, curves right | Path work first; you are likely cutting across it with a face open to that path |
| Starts right, curves farther right | Face control first, then confirm the path |
| Starts left and stays left | Face may be closed to target; do not strengthen the grip more |
| Starts right and stays right | Face and path may both be right; close the face relative to path |
| Alternates hook and slice | Stop making random fixes and get face/path measured in a lesson |

The goal is not to force a draw immediately. First turn the giant curve into a playable fade. Then, if you want to move the path farther right, use the step-by-step [draw guide](/blog/how-to-hit-a-draw-step-by-step/).
