...
Before drilling anything, hit five balls and write down only two things: **where the ball started** and **which way it curved**. Those two observations tell you which relationship needs work.

| Ball flight for a right-handed golfer | What it usually means | What to change first |
|---|---|---|
| Starts right, curves left | Face right of target but closed to path | That is the playable draw; manage the amount of curve |
| Starts right, stays right | Face and path both right with little difference | Close the face slightly relative to the path |
| Starts left, curves farther left | Face left of target and closed to path | Neutralize the grip/face before changing path |
| Starts left, curves right | Face open to an out-to-in path | Fix the path and face together with the [slice guide](/blog/how-to-fix-a-slice-for-good/) |
| Starts near target, hooks hard | Path too far right or face closing too fast | Reduce the path/face gap; stop trying to manufacture extra curve |

Left-handed golfers can reverse the directions. The relationship is the same: the face largely controls start direction, while the face-to-path difference creates the curve.
...
Build the change inside a structured [practice session](/blog/how-to-practice-with-purpose/), then take it to the course with the commitment rules in [playing under pressure](/blog/how-to-play-golf-under-pressure/). If the face-to-path diagnosis still feels like algebra after two focused sessions, book a lesson and get measured instead of guessing for another month.
